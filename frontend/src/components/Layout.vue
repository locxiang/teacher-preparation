  ...
  <div class="min-h-screen flex flex-col bg-gray-50 font-sans">
      ...
      <div class="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        >
          ...
          <div>
            ...
            <p class="text-xs text-nanyu-100 opacity-90">
              集体备课 AI 助手
            </p>
          </div>
        </router-link>
        ...
        <nav class="hidden md:flex items-center space-x-6">
          <template v-if="authStore.isAuthenticated.value">
            <router-link
              to="/"
              class="hover:text-nanyu-100 transition-colors font-medium"
            >
              首页
            </router-link>
            <router-link
            ...
            <div class="flex items-center space-x-2 ml-4 pl-4 border-l border-nanyu-500">
              <router-link
                to="/profile"
                class="flex items-center space-x-2 hover:text-nanyu-100 transition-colors cursor-pointer"
              >
    ...
    <footer class="bg-gray-800 text-gray-400 py-6 mt-auto">
      <div class="container mx-auto px-4 text-center">
        <p class="text-sm">
          &copy; {{ new Date().getFullYear() }} 重庆南渝中学. All rights reserved.
        </p>
        <p class="text-xs mt-1 opacity-60">
          Powered by AI Teaching Assistant System
        </p>
      </div>
    </footer>
  </div>
